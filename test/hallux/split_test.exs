  ...
  describe "split_tree" do
    ...
    test "empty", %{p: p, mfn: mfn, rfn: rfn, z: z} do
      assert_raise FunctionClauseError, fn ->
        Split.split_tree(p, z, empty(), z, mfn, rfn)
      end
    end

    test "single", %{p: p, mfn: mfn, rfn: rfn, z: z} do
      assert split(empty(), 4, empty()) == Split.split_tree(p, z, single(4), z, mfn, rfn)
    end
    ...
    test "deep", %{p: p, mfn: mfn, rfn: rfn, z: z} do
      ...
      assert split(
               deep(four(6, 7, 8, 9), empty(), one(10), z, mfn, rfn),
               11,
               deep(one(12), single(node3(13, 14, 15, z, mfn, rfn)), one(16), z, mfn, rfn)
  ...
  describe "split_tree : random access" do
    ...
    test "random access", %{p: p, mfn: mfn, rfn: rfn, z: z} do
      ...
      assert split(
               ...
               17,
               deep(one(18), single(node3(19, 20, 21)), one(22))
             ) == Split.split_tree(p, z, tree, z, mfn, rfn)
    end
  ...
  describe "split_digit" do
    ...
    test "four", %{p: p, mfn: mfn, rfn: rfn} do
      ...
      assert split([10], 11, [12, 13]) ==
               Split.split_digit(p, 0, four(10, 11, 12, 13), 0, mfn, rfn)

      assert split([], 11, [12, 13, 14]) ==
               Split.split_digit(p, 0, four(11, 12, 13, 14), 0, mfn, rfn)
    end
  end
end
