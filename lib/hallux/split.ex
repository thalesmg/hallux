defmodule Hallux.Split do
  ...
  def split(tree, predicate, zero, measure_fn, reduce_fn) when is_function(predicate, 1) do
    %__MODULE__{l: l, x: x, r: r} = split_tree(predicate, zero, tree, zero, measure_fn, reduce_fn)
  ...
  def split_tree(predicate, acc, %Deep{} = tree, zero, measure_fn, reduce_fn) do
    ...
    vpr =
      Measured.size(pr, zero, measure_fn, reduce_fn)
      |> reduce_fn.(acc)
    ...
    cond do
      predicate.(vpr) ->
        %__MODULE__{l: l, x: x, r: r} =
          split_digit(predicate, acc, pr, zero, measure_fn, reduce_fn)

        r_ = deepL(r, m, sf, zero, measure_fn, reduce_fn)
        %__MODULE__{l: Internal.to_tree(l), x: x, r: r_}

      predicate.(vm) ->
        %__MODULE__{l: ml, x: xs, r: mr} =
          split_tree(predicate, vpr, m, zero, measure_fn, reduce_fn)
        ...
        %__MODULE__{l: l, x: x, r: r} =
          split_digit(predicate, ml_acc, Node.to_digit(xs), zero, measure_fn, reduce_fn)
      ...
      :otherwise ->
        %__MODULE__{l: l, x: x, r: r} =
          split_digit(predicate, vm, sf, zero, measure_fn, reduce_fn)
  ...
  def split_digit(_predicate, _acc, %One{a: a}, _, _, _), do: %__MODULE__{l: [], x: a, r: []}
  ...
  def split_digit(predicate, acc, digit, zero, measure_fn, reduce_fn) do
    ...
    if predicate.(acc) do
      ...
      %__MODULE__{l: l, x: x, r: r} =
        split_digit(predicate, acc, Digits.to_digits(rest), zero, measure_fn, reduce_fn)
